...
namespace TelegramBot.Common
{
    public static class DataService
    {
        public static async Task LoadAppData()
        {
            ...
            {
                CurrentDialogBotData.DialogBotData = new DialogBotData()
                {
                    AnswerData = new ConcurrentDictionary<long, ConcurrentQueue<string>>(),
                    QuestionsData = new ConcurrentDictionary<long, ConcurrentQueue<string>>()
                };
            }

            ChatSettings.ChatSettingsData =
                await LoadData<ConcurrentQueue<ChatSettingsBotData>>(BotConstants.Paths.Settings) ??
                new ConcurrentQueue<ChatSettingsBotData>();
        }
        ...
        {
            var difference = CurrentDialogBotData.DialogBotData.QuestionsData.Count -
                             CurrentDialogBotData.DialogBotData.AnswerData.Count;
            ...
            if (difference > 0)
            {
                ...
                {
                    CurrentDialogBotData.DialogBotData.QuestionsData.TryRemove(
                        CurrentDialogBotData.DialogBotData.QuestionsData.Count - 1 - i, out var result);
                }
            }
            else if (difference < 0)
            {
                for (var i = 0; i < -difference; i++)
                {
                    CurrentDialogBotData.DialogBotData.AnswerData.TryRemove(
                        CurrentDialogBotData.DialogBotData.AnswerData.Count - 1 - i, out var result);
                }
            }
