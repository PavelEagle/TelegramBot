    ...
    public sealed class DefaultTextCommand : ITextCommand
    {
        ...
        public async Task ProcessMessage(Message message)
        {
            if (CurrentDialogBotData.DialogBotData.QuestionsData == null ||
                CurrentDialogBotData.DialogBotData.AnswerData == null)
            {
            ...
            var questionId = CurrentDialogBotData.DialogBotData.QuestionsData
                .Where(x => x.Value.Contains(message.Text.ToLower())).Select(x => x.Key).FirstOrDefault();
            ...
            {
                var answerData =
                    CurrentDialogBotData.DialogBotData.AnswerData.SingleOrDefault(x => x.Key == questionId);
            ...
            if (_chatSettingsBotData.VoiceAnswer)
            {
                ...
                await _botService.Client.SendAudioAsync(message.Chat.Id,
                    new InputOnlineFile(new MemoryStream(voiceMessage)));
                return;
            }
